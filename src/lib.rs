...
impl From<Vec<u8>> for Value {
    fn from(b: Vec<u8>) -> Self { Value::String(b) }
}
...
impl StorageEngine {
    ...
    pub async fn start_expiration_sweeper(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let expired: Vec<(Instant, Vec<String>)> = e.iter()
                    .filter(|(t, _)| **t <= now)
    ...
    pub fn set(&self, key: &str, value: RedisData, expire_at: Option<Instant>) {
        ...
        let stored = StoredValue { data: value, expire_at };
...
impl ToRedisArgs for String {
    fn to_redis_args(&self) -> Vec<Value> { vec![Value::String(self.as_bytes().to_vec())] }
}
...
impl ToRedisArgs for Vec<u8> {
    fn to_redis_args(&self) -> Vec<Value> { vec![Value::String(self.clone())] }
}

impl ToRedisArgs for i64 {
    fn to_redis_args(&self) -> Vec<Value> { vec![Value::Int(*self)] }
}

impl ToRedisArgs for u64 {
    fn to_redis_args(&self) -> Vec<Value> { vec![Value::Int(*self as i64)] }
}
...
impl ToRedisArgs for bool {
    fn to_redis_args(&self) -> Vec<Value> { vec![Value::Bool(*self)] }
}

impl<T: ToRedisArgs> ToRedisArgs for Option<T> {
    fn to_redis_args(&self) -> Vec<Value> {
        match self { Some(v) => v.to_redis_args(), None => vec![Value::Null] }
    }
}
...
impl FromRedisValue for Vec<u8> {
    fn from_redis_value(v: Value) -> RedisResult<Self> {
        match v { Value::String(s) => Ok(s), _ => Err(RedisError::ParseError) }
    }
}
...
impl FromRedisValue for i64 {
    fn from_redis_value(v: Value) -> RedisResult<Self> {
        match v {
            Value::Int(n) => Ok(n),
            Value::String(s) => String::from_utf8(s).ok().and_then(|s| s.parse().ok()).ok_or(RedisError::ParseError),
...
impl FromRedisValue for Value {
    fn from_redis_value(v: Value) -> RedisResult<Self> { Ok(v) }
}
...
impl Client {
    pub fn new() -> Self {
        Self { storage: StorageEngine::new() }
    }
    ...
    where K: ToRedisArgs, RV: FromRedisValue {
        ...
        if let Some(val) = self.storage.get(&key_str) {
            if val.is_expired() { self.storage.remove(&key_str); return FromRedisValue::from_redis_value(Value::Null); }
            match val.data { RedisData::String(s) => RV::from_redis_value(Value::String(s)), _ => Err(RedisError::WrongType) }
        } else { FromRedisValue::from_redis_value(Value::Null) }
    }
    ...
    where K: ToRedisArgs {
        Ok(self.storage.set_expiry(&Self::key_to_string(&key), Duration::from_secs(seconds as u64)))
    }
    ...
    where K: ToRedisArgs, F: ToRedisArgs, V: ToRedisArgs {
        let key_str = Self::key_to_string(&key);
        ...
        let is_new = if let Some(val) = self.storage.get(&key_str) {
            match val.data {
                RedisData::Hash(mut h) => {
                    ...
                    self.storage.set(&key_str, RedisData::Hash(h), val.expire_at);
                    is_new
                }
    ...
    where K: ToRedisArgs, F: ToRedisArgs, RV: FromRedisValue {
        ...
        if let Some(val) = self.storage.get(&key_str) {
            ...
            match val.data {
                RedisData::Hash(h) => {
                    if let Some(v) = h.get(&field_b) { RV::from_redis_value(Value::String(v.clone())) }
                    else { FromRedisValue::from_redis_value(Value::Null) }
                }
    ...
    where K: ToRedisArgs, RV: FromRedisValue {
        let key_str = Self::key_to_string(&key);
        if let Some(val) = self.storage.get(&key_str) {
            if val.is_expired() { self.storage.remove(&key_str); return FromRedisValue::from_redis_value(Value::Array(Vec::new())); }
            match val.data {
                RedisData::Hash(h) => {
                    let mut res = Vec::new();
                    for (k, v) in h.iter() { res.push(Value::String(k.clone())); res.push(Value::String(v.clone())); }
                    FromRedisValue::from_redis_value(Value::Array(res))
                }
                _ => Err(RedisError::WrongType),
            }
        } else { FromRedisValue::from_redis_value(Value::Array(Vec::new())) }
    }
        ...
        if let Some(val) = self.storage.get(&key_str) {
            match val.data {
                RedisData::Hash(mut h) => {
                    ...
                    if existed { self.storage.set(&key_str, RedisData::Hash(h), val.expire_at); }
        ...
        let len = if let Some(val) = self.storage.get(&key_str) {
            match val.data {
                RedisData::List(mut l) => {
                    l.push_front(val_b);
                    self.storage.set(&key_str, RedisData::List(l.clone()), val.expire_at);
                    l.len() as i64
                }
                _ => return Err(RedisError::WrongType),
            }
    ...
    where K: ToRedisArgs {
        let key_str = Self::key_to_string(&key);
        if let Some(val) = self.storage.get(&key_str) {
            if val.is_expired() { self.storage.remove(&key_str); return Ok(0); }
            match val.data { RedisData::List(l) => Ok(l.len() as i64), _ => Err(RedisError::WrongType) }
        } else { Ok(0) }
    }
    ...
    pub async fn ping(&mut self) -> RedisResult<String> { Ok("PONG".to_string()) }
    ...
    pub async fn dbsize(&mut self) -> RedisResult<i64> { Ok(self.storage.len() as i64) }
